...
use dioxus::prelude::*;
use filen_sdk_rs::{
	auth::{http::ClientConfig, unauth::UnauthClient, Client},
	fs::categories::{DirType, NonRootFileType},
	io::{client_impl::IoSharedClientExt, RemoteDirectory},
};
use rusqlite::Connection;

use crate::{
	backend::util::UnwrapOnceLock,
...
impl DbViaOfflineOrRemoteFile {
	...
	fn init(db_dir: Option<&str>) -> Connection {
		let db_dir = db_dir.unwrap_or(".").trim_end_matches('/');
	...
	async fn initialize_from_filen(client: &Client) -> anyhow::Result<RemoteDirectory> {
		...
		if tokio::fs::try_exists(&local_db_file)
			.await
			.context("Failed to check if local database file exists")?
		{
			tokio::fs::remove_file(&local_db_file)
				.await
				.context("Failed to remove existing local database file")?;
		}
		match client
			...
			.await?
		{
			Some(NonRootFileType::File(db_file)) => {
				client
					.download_file_to_path(db_file.as_ref(), local_db_file, None)
					.await?;
			}
			_ => {
			...
			.context("Failed to create .filen-relay dir in admin Filen account")?
		{
			...
			Err(anyhow::anyhow!(
				"Failed to find or create .filen-relay dir in admin Filen account"
			))
		}
	}
	...
	async fn write_to_filen(&self) -> anyhow::Result<()> {
		...
		client
			.upload_file_from_path(
				&DirType::Dir(std::borrow::Cow::Borrowed(
